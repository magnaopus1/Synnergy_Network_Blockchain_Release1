...
// WalletRecoveryService provides methods to generate and recover wallets using mnemonics
type WalletRecoveryService struct{}

// NewWalletRecoveryService initializes and returns a new WalletRecoveryService
func NewWalletRecoveryService() *WalletRecoveryService {
	return &WalletRecoveryService{}
}

// GenerateMnemonic generates a new 12-word mnemonic phrase
func (wrs *WalletRecoveryService) GenerateMnemonic() (string, error) {
...
func (wrs *WalletRecoveryService) MnemonicToSeed(mnemonic, passphrase string) ([]byte, error) {
	return bip39.NewSeedWithErrorChecking(mnemonic, passphrase)
}

// ValidateMnemonic checks if a given mnemonic phrase is valid
func (wrs *WalletRecoveryService) ValidateMnemonic(mnemonic string) bool {
	return bip39.IsMnemonicValid(mnemonic)
}

// EncryptMnemonic encrypts the mnemonic with a passphrase using Argon2
func (wrs *WalletRecoveryService) EncryptMnemonic(mnemonic, passphrase string) (string, error) {
	salt := make([]byte, 16)
...
func (wrs *WalletRecoveryService) DecryptMnemonic(encrypted, passphrase string) (string, error) {
...
func (wrs *WalletRecoveryService) RecoverWalletFromMnemonic(mnemonic, passphrase string) ([]byte, error) {
	if !wrs.ValidateMnemonic(mnemonic) {
		return nil, errors.New("invalid mnemonic phrase")
	}
	return wrs.MnemonicToSeed(mnemonic, passphrase)
}
...
func (brs *BlockchainRecoveryService) RecoverBlockchainState(mnemonic, passphrase string) ([]byte, error) {
	seed, err := brs.wrs.RecoverWalletFromMnemonic(mnemonic, passphrase)
	if err != nil {
		return nil, err
	}
