...
// ContractManager is responsible for managing smart contracts
...
func NewContractManager(keystoreDir, keystorePass, rpcURL string) *ContractManager {
...
func (cm *ContractManager) DeployContract(binaryPath string) (string, error) {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	...
	rawTx, err := cm.createSignedTransaction(privateKey, contractBin)
	if err != nil {
		return "", fmt.Errorf("failed to create signed transaction: %w", err)
	}

	// Send the transaction
	txHash, err := cm.sendTransaction(rawTx)
	if err != nil {
		return "", fmt.Errorf("failed to send transaction: %w", err)
	}

	// Wait for the transaction to be mined and get the contract address
	contractAddress, err := cm.waitForTransaction(txHash)
	if err != nil {
...
func (cm *ContractManager) getPrivateKey() (*ecdsa.PrivateKey, error) {
...
// decryptKey decrypts the keystore JSON file to retrieve the private key
func decryptKey(keyJSON []byte, password string) (*ecdsa.PrivateKey, error) {
	...
	privateKey, err := bytesToPrivateKey(plainText)
	if err != nil {
...
// aesDecrypt decrypts the ciphertext using the given key and iv
...
// bytesToPrivateKey converts bytes to ECDSA private key
func bytesToPrivateKey(privateKeyBytes []byte) (*ecdsa.PrivateKey, error) {
	x, y := elliptic.Unmarshal(elliptic.P256(), privateKeyBytes)
	if x == nil || y == nil {
...
func (cm *ContractManager) createSignedTransaction(privateKey *ecdsa.PrivateKey, contractBin []byte) (string, error) {
...
// signTransaction signs the transaction hash using the private key
func signTransaction(hash []byte, privateKey *ecdsa.PrivateKey) (signature struct{ R, S, V *big.Int }, err error) {
	r, s, err := ecdsa.Sign(bytes.NewReader(hash), privateKey, hash)
	if err != nil {
...
// sendTransaction sends the raw transaction to the blockchain
func (cm *ContractManager) sendTransaction(rawTx string) (string, error) {
	...
	req, err := http.NewRequest("POST", cm.rpcURL, bytes.NewBuffer(body))
	if err != nil {
...
// waitForTransaction waits for the transaction to be mined and returns the contract address
func (cm *ContractManager) waitForTransaction(txHash string) (string, error) {
	for {
...
// getTransactionCount gets the transaction count for the given private key
func (cm *ContractManager) getTransactionCount(privateKey *ecdsa.PrivateKey) (uint64, error) {
...
// getTransactionReceipt gets the transaction receipt for the given transaction hash
func (cm *ContractManager) getTransactionReceipt(txHash string) (*Receipt, error) {
	payload := map[string]interface{}{
		"jsonrpc": "2.0",
