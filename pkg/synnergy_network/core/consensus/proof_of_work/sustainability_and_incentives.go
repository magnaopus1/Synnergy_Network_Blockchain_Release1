...
func NewSustainabilityAndIncentives(blockchain *common.Blockchain) *SustainabilityAndIncentives {
...
func (sai *SustainabilityAndIncentives) AdjustBlockReward(blockHeight int64) {
	...
	if reductionFactor > 0 {
		...
		if newReward.Cmp(big.NewInt(1)) >= 0 {
			sai.Blockchain.Reward = newReward
		} else {
			// Minimum reward threshold
			sai.Blockchain.Reward = big.NewInt(1)
		}
	}
}
