...
func (d *DHT) DiscoverPeers(targetID string) ([]*Node, error) {
	...
	// Sort nodes by distance from the target node.
	sort.Slice(closestNodes, func(i, j int) bool {
		nodeIntI := new(big.Int)
		nodeIntI.SetString(closestNodes[i].ID, 16)
		distanceI := new(big.Int)
		distanceI.Xor(targetInt, nodeIntI)

		nodeIntJ := new(big.Int)
		nodeIntJ.SetString(closestNodes[j].ID, 16)
		distanceJ := new(big.Int)
		distanceJ.Xor(targetInt, nodeIntJ)

		return distanceI.Cmp(distanceJ) == -1
	})
