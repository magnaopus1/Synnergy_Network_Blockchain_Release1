...
# health_check.sh - A comprehensive health check script for Synthron Gateway Node

# Load configuration variables
CONFIG_PATH="/etc/gateway_node/config.toml"
source $CONFIG_PATH

# Log file for health check output
LOG_FILE="/var/log/gateway_node/health_check.log"
...
check_disk_space() {
    THRESHOLD=90
    USAGE=$(df -h | grep '/var/lib/gateway_node/data' | awk '{print $5}' | sed 's/%//')
