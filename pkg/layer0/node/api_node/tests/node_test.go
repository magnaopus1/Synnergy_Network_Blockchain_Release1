...
import (
	...
	"github.com/synthron_blockchain/pkg/layer0/node/api_node"
)

func setupTestNode(t *testing.T) *api_node.APINode {
	...
	node, err := api_node.NewAPINode("test-node", "127.0.0.1:8080", privateKey, publicKey)
	if err != nil {
		t.Fatalf("Failed to create APINode: %v", err)
	}

	return node
}

func TestAPINode_StartStop(t *testing.T) {
	node := setupTestNode(t)

	go func() {
		err := node.Start()
		if err != nil && err != context.Canceled {
			t.Fatalf("Failed to start APINode: %v", err)
		}
	}()
...
func TestAPINode_EncryptDecryptData(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_AuthenticateVerify(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_Integration(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_PerformSecurityAudit(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_SendTransaction(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_QueryBlockchain(t *testing.T) {
	node := setupTestNode(t)
...
func TestAPINode_EnhancedTransactionRouting(t *testing.T) {
	node := setupTestNode(t)
